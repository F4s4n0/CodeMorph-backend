...
def get_understanding_tasks(agents, output_dir, numero_file=0):
    ...
    vincolo_ampiezza = (
        ...
        "hai riassunto invece di analizzare. Per ogni area funzionale spiega "
        "COME funziona il meccanismo, non solo che esiste — quali dati "
        "attraversano quali moduli, in che ordine, con quali condizioni e "
        "quali casi limite. Cita il codice reale: nomi di funzioni, tabelle "
        "interrogate, valori cablati che hai trovato.\n"
        "L'unico limite e' la SOSTANZA: non aggiungere prassi generiche di "
        "settore o paragrafi che varrebbero per qualunque sistema. Tutto cio' "
        "che scrivi deve venire dal codice che hai letto.\n"
        "COMPLETEZZA: il documento deve essere autoconclusivo. Non annunciare "
        "nell'indice sezioni che non svilupperai e non inserire rinvii a "
        "sezioni inesistenti."
    )
    if numero_file:
    ...
    documentation_task = Task(
        description=(
            ...
            "PROFONDITA' PER MODULO: ogni modulo funzionale va documentato al "
            "livello del MECCANISMO, non dell'elenco. Per ciascuno spiega come "
            "funziona il flusso: quali dati entrano, attraverso quali file "
            "passano, in che ordine, con quali condizioni e casi limite. "
            "Riporta le formule di calcolo, le soglie, i codici e i valori "
            "cablati che hai trovato nel sorgente.\n"
            "Le tabelle di mappatura degli asset sono un COMPLEMENTO utile, ma "
            "non sostituiscono la spiegazione: una riga che elenca diciassette "
            "form con la nota 'CRUD anagrafico' dice al lettore che quei file "
            "esistono, non cosa fanno. Se un modulo raccoglie decine di file, "
            "descrivi il meccanismo che li accomuna e poi i casi che se ne "
            "discostano, invece di ridurli tutti a una casella di tabella.\n"
            "Il criterio: chi legge deve poter riscrivere quel modulo senza "
            "avere il codice originale sotto gli occhi."
            + MERMAID_RULES + CONVENZIONI_FASE1
        ),
        expected_output=(
    ...
    functional_analysis_task = Task(
        description=(
            ...
            "QUANTE USER STORY: il backlog deve coprire i comportamenti che il "
            "sistema espone davvero, non le sole macro-funzionalita'. Ogni Epic "
            "raccoglie decine di file legacy: una manciata di story per Epic "
            "significa che hai descritto il titolo del modulo, non cosa fa. "
            "Scendi al livello dell'AZIONE che l'utente compie — inserire una "
            "presenza, correggere una timbratura, approvare un giustificativo, "
            "esportare il file paghe — e scrivi una story per ciascuna.\n"
            "I criteri di accettazione devono contenere i VALORI REALI trovati "
            "nel codice: soglie, formati, codici di errore, regole di calcolo. "
            "Un criterio generico ('il sistema mostra un messaggio di errore') "
            "non permette a nessuno di verificare la migrazione; uno specifico "
            "('se le ore superano 8 il sistema calcola lo straordinario al 15%') "
            "si'.\n"
            "Se una regola di business e' complessa, dedicale una story propria "
            "e spiegala per intero: chi riscrivera' il sistema non avra' il "
            "codice originale sotto gli occhi."
            + MERMAID_RULES + CONVENZIONI_FASE1
        )+ _nota_data(),
        expected_output=(
    ...
    test_book_task = Task(
        ...
        expected_output=(
            ...
            "coinvolta.\n"
            "QUANTI CASI: tanti quanti servono a coprire i comportamenti "
            "osservati, non un numero prefissato. Ogni area funzionale del "
            "sistema deve avere i suoi casi, e le aree che concentrano logica "
            "di business (calcoli, regole, integrazioni) ne richiedono di piu'. "
            "Un piano con quindici casi su un sistema di centinaia di file "
            "lascia scoperta la maggior parte dei comportamenti da verificare: "
            "e' proprio dove non hai scritto un test che la migrazione "
            "introdurra' la regressione che nessuno trovera'."
        )+ _nota_data(),
  ...
  # Il contenuto dei documenti arriva via inputs al kickoff, NON incollato
    ...
    dba_task = Task(
        description=(
            ...
            "   Lo script deve essere RIESEGUIBILE su un database che potrebbe\n"
            "   gia' contenere alcune di queste tabelle: proteggi ogni CREATE con\n"
            "   un controllo di esistenza. Un sistema che usa gia' un database\n"
            "   relazionale non parte da zero, e uno script che fallisce alla\n"
            "   prima istruzione non e' utilizzabile.\n"
...
{blocco_tipi_esistenti}
        ...
        expected_output=(
            ...
            "arrivera' mai, e un'interfaccia registrata nella dependency "
            "injection senza implementazione impedisce all'applicazione di "
            "AVVIARSI. Se ti serve un contratto che esiste gia' (repository, "
            "servizi, contesti dati), RIUSALO invece di dichiararne uno nuovo; "
            "se non esiste e non puoi implementarlo qui, non dichiararlo e usa "
            "direttamente il tipo concreto.\n"
...
def get_quality_check_task(
    ...
    quality_check_task = Task(
        description=(
            ...
            "CONTROLLA IN PARTICOLARE I DIFETTI CHE IMPEDISCONO L'AVVIO:\n"
            "- interfacce dichiarate senza alcuna classe che le implementi: "
            "registrate nella dependency injection, bloccano l'applicazione "
            "all'avvio e non in compilazione, quindi passano inosservate;\n"
            "- tipi definiti piu' volte con lo stesso nome in file diversi;\n"
            "- riferimenti a classi, servizi o namespace che non compaiono in "
            "nessuna parte del codice generato.\n"
            "Sono difetti piu' gravi di una vulnerabilita' teorica: rendono il "
            "progetto inutilizzabile finche' non vengono risolti, e vanno "
            "segnalati come BLOCCANTI.\n\n"
...
def get_validation_task(agent, output_dir, nome_fase, output_filename):
    ...
    return [Task(
        description=(
            ...
            "   Se fra i documenti c'e' un backlog funzionale, verifica che le "
            "user story scendano al livello dell'AZIONE dell'utente e non si "
            "fermino alle macro-funzionalita': poche story per Epic su un "
            "sistema di centinaia di file significano che i comportamenti "
            "reali non sono stati descritti. Controlla anche che i criteri di "
            "accettazione contengano valori concreti (soglie, formati, regole) "
            "e non formule generiche.\n"
            "   Guarda il rapporto fra FILE CITATI e TESTO SCRITTO in ciascuna "
            "sezione: una sezione che nomina decine di file in poche righe li "
            "sta elencando, non documentando. E' il segnale piu' affidabile di "
            "un'analisi superficiale — segnala quali sezioni ne soffrono.\n"
            "   Valuta la PROFONDITA' rispetto alla dimensione del sistema: se i "
            "documenti descrivono centinaia di file in poche pagine, l'analisi "
            "e' riassuntiva e non approfondita — segnalalo come rilievo, "
            "indicando quali aree meriterebbero un livello di dettaglio "
            "maggiore (meccanismi di calcolo, regole di business, casi limite). "
            "Un documento breve non e' un pregio quando il sistema e' grande.\n"
            ...
            "scoprira' mancanti solo a migrazione avvenuta. Verifica anche che "
            "lo script sia eseguibile su un database che potrebbe gia' contenere "
            "alcune tabelle: un DDL che fallisce alla prima istruzione non e' "
            "utilizzabile.\n"
